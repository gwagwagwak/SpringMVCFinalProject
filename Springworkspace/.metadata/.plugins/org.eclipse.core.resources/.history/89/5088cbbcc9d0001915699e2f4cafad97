...
/*
  페이징 처리가 필요할  Controller 구문에 
  
  @RequestParam으로 인자를 받는다.
  총 레코드 갯수를 따로 구한 뒤 
...
public class Pager {
	...
	private int curBlock; // 현재 페이지 브록
	...
	public void setBlockRange() {
		curBlock = (int) Math.ceil((curPage - 1) / BLOCK_SCALE) + 1; // 현재 페이지가
																		// 몇 번째
																		// 페이지
																		// 블록에
																		// 속하는지
		...
		if (blockEnd > totPage)
			blockEnd = totPage; // 마지막 페이지 번호가 블록 범위 초과하지 않도록함

		prevPage = (curBlock == 1) ? 1 : (curBlock - 1) * BLOCK_SCALE; // [이전]
																		// 눌럿을 때
																		// 이동할
																		// 페이지
																		// 번호
		nextPage = curBlock > totBlock ? (curBlock * BLOCK_SCALE) : curBlock * BLOCK_SCALE; // [다음]눌럿을
																							// 때
																							// 이동할
																							// 페이지
																							// 번호
		...
		if (nextPage >= totPage)
			nextPage = totPage; // 마지막 페이지가 페이지 범위 초과하지 않도록
	}
